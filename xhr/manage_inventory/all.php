...
// =====================================================
// STEPS 12-20: MISSING RESCHEDULE API ENDPOINTS
// =====================================================

    // STEP 12: Get reschedule context for modal
    if ($s === 'get_reschedule_context') {
        ...
        try {
            ...
            $total_amount = intval($helper->total_price ?? 0);
            $total_paid = intval($helper->total_paid ?? 0);
            $remaining = $total_amount - $total_paid;
            
            // Get schedule data
            $schedule = $db->where('purchase_id', $purchase_id)->orderBy('installment', 'ASC')->get('crm_payment_schedule');
            ...
            if (!empty($schedule)) {
                foreach ($schedule as $row) {
                    if ($row->status != 1) {
                        $unpaid_rows++;
                        if ($current_monthly == 0) $current_monthly = intval($row->installment_amount ?? 0);
                    } else {
                        $last_paid_date = $row->payment_date;
                    }
                }
            }
            ...
            echo json_encode([
                ...
                'context' => [
                    ...
                    'last_paid_date' => $last_paid_date ? date('d-m-Y', strtotime($last_paid_date)) : 'No payment yet'
                ]
            ]);
        } catch (Exception $e) {
    ...
    // STEP 13: Preview reschedule before submission
    if ($s === 'preview_reschedule') {
        header('Content-Type: application/json; charset=utf-8');
        
        $purchase_id = isset($_POST['purchase_id']) ? (int)$_POST['purchase_id'] : 0;
        $monthly_amount = isset($_POST['monthly_amount']) ? floatval($_POST['monthly_amount']) : 0;
        $adjustment_mode = isset($_POST['adjustment_mode']) ? trim($_POST['adjustment_mode']) : 'last_installment';
        
        try {
            $validation = validateRescheduleRequest($purchase_id, $monthly_amount, $adjustment_mode);
            if (!$validation['valid']) {
                echo json_encode(['status' => 400, 'message' => $validation['error']]);
                exit;
            }
            ...
            foreach ($schedule as $i => $row) {
                if ($row->status == 1) {
                    $last_paid_index = $i;
                    $last_paid_date = $row->payment_date;
                } else if ($current_monthly == 0) {
                    $current_monthly = intval($row->installment_amount ?? 0);
                }
            }
            
            // Build preview schedule
            $new_schedule = [];
            $current_date = new DateTime($last_paid_date ?: date('Y-m-d'));
            $current_date->modify('+1 month');
            $total_new_installments = ceil($remaining_balance / $monthly_amount);
            
            for ($i = 0; $i < $total_new_installments; $i++) {
                $amount = intval($monthly_amount);
                
                if ($i == $total_new_installments - 1) {
                    $current_sum = $i * intval($monthly_amount);
                    $amount = max(0, $remaining_balance - $current_sum);
                }
            ...
            echo json_encode([
                ...
                'preview' => [
                    'before' => [
                        ...
                        'unpaid_rows' => max(0, count($schedule) - $last_paid_index - 1),
                        'last_paid_date' => $last_paid_date ? date('d-m-Y', strtotime($last_paid_date)) : 'N/A'
                    ],
                    'after' => [
                        'monthly_amount' => intval($monthly_amount),
                        'remaining_balance' => $remaining_balance,
                        'new_installment_count' => $total_new_installments,
                        'adjustment_mode' => $adjustment_mode,
                        'new_schedule' => array_slice($new_schedule, 0, 8)
                    ]
                ],
                'requires_approval' => !Wo_IsAdmin() && $monthly_amount >= 50000
            ]);
        } catch (Exception $e) {
            echo json_encode(['status' => 500, 'message' => 'Preview error: ' . $e->getMessage()]);
        }
        exit;
    }

    // STEP 14: Submit payment reschedule request
        ...
        try {
            ...
            if ($requires_approval) {
                ...
                $pending_data = [
                    ...
                    'requested_by' => $GLOBALS['wo']['user']['id'] ?? null,
                    'status' => 'pending',
                    'created_at' => date('Y-m-d H:i:s')
                ];
                ...
                // Send notification
                sendRescheduleNotification($purchase_id, $helper->client_id, 'submitted', [
                    'monthly_amount' => $monthly_amount,
                    'adjustment_mode' => $adjustment_mode,
                    'reason' => $reason
                ]);
                ...
                // Execute directly (admin or low amount)
                executeApprovedReschedule($purchase_id, $monthly_amount, $adjustment_mode, $reason);
    ...
    // STEP 15: Get reschedule history for a purchase
    if ($s === 'get_reschedule_history') {
        ...
        try {
            ...
            $history = $db->where('purchase_id', $purchase_id)->orderBy('reschedule_date', 'DESC')->get('crm_payment_reschedule_history');
            
            $result = [];
            if (!empty($history)) {
                foreach ($history as $record) {
                    $change_data = @json_decode($record->change_data ?? '{}', true) ?? [];
                    if (empty($change_data)) {
                        $change_data = [
                            'monthly_amount' => $record->monthly_installment_amount ?? 0,
                            'adjustment_mode' => $record->adjustment_mode ?? 'last_installment',
                            'total_adjustments' => 0
                        ];
                    }
                    ...
                    $result[] = [
                        'id' => $record->id,
                        'reschedule_date' => $record->reschedule_date,
    ...
    // STEP 16: Get pending changes list
    if ($s === 'get_pending_changes') {
    
        $purchase_id = isset($_POST['purchase_id']) ? (int) $_POST['purchase_id'] : 0;
        $status_raw   = isset($_POST['status']) ? $_POST['status'] : 'pending';
    
        try {
            // Normalize status: allow string or array of strings (for IN queries)
            $status = $status_raw;
            if (!is_array($status)) {
                // Trim and coerce to string if it's scalar
                if (is_scalar($status)) $status = trim((string)$status);
                else $status = 'pending';
            } else {
                // Ensure all entries are strings
                $status = array_map(function($v){ return trim((string)$v); }, $status);
            }
    
            // Build WHERE clauses safely: call where() per column (avoids accidental 'Array' injection)
            if (is_array($status)) {
                // This produces WHERE `status` IN (..)
            ...
            $changes = $db->get('crm_pending_changes');
    
            $result = [];
            if (!empty($changes)) {
                foreach ($changes as $change) {
                    // decode JSON safely
                    $raw_json = isset($change->change_data_json) && $change->change_data_json !== null
                               ? $change->change_data_json
                               : '{}';
                    $change_data = json_decode($raw_json, true);
                    if (!is_array($change_data)) $change_data = [];
    
                    // Try to resolve requester name from crm_customers (fallback to id)
                    $requested_by_name = (string) ($change->requested_by ?? '0');
                    if (!empty($change->requested_by) && is_numeric($change->requested_by)) {
                        // fetch customer record (returns object rows in your project)
                        $db->where('id', (int)$change->requested_by);
                        $requester = $db->getOne('crm_customers');
                        if ($requester && isset($requester->name) && $requester->name !== '') {
                    ...
                    $result[] = [
                        ...
                        'request_reason' => $change->request_reason ?? ($change_data['reason'] ?? ''),
                        'change_data' => $change_data,
            ...
            http_response_code(200);
            echo json_encode(['status' => 200, 'changes' => $result], JSON_UNESCAPED_UNICODE);
        } catch (Exception $e) {
            http_response_code(500);
            echo json_encode(['status' => 500, 'message' => 'Error: ' . $e->getMessage()], JSON_UNESCAPED_UNICODE);
        }
        exit;
    }

    // STEP 17: Approve pending change (admin only)
    if ($s === 'approve_pending_change') {
        ...
        try {
            $change = $db->where('id', $change_id)->getOne('crm_pending_changes');
            if (!$change || $change->status != 'pending') {
                echo json_encode(['status' => 404, 'message' => 'Change not found or not pending']);
                exit;
            }
            ...
            if ($change->change_type === 'reschedule') {
                ...
                executeApprovedReschedule(
                    $change->purchase_id,
                    $data['monthly_amount'] ?? 0,
                    $data['adjustment_mode'] ?? 'last_installment',
                    $data['reason'] ?? ''
                );
            }
            
            // Update pending change status
            $db->where('id', $change_id)->update('crm_pending_changes', [
                'status' => 'approved',
                'review_date' => date('Y-m-d H:i:s'),
                'reviewed_by' => $GLOBALS['wo']['user']['id'] ?? null,
                'reviewed_by_name' => $GLOBALS['wo']['user']['name'] ?? 'Admin'
            ]);
            
            // Notify requester
            sendRescheduleNotification($change->purchase_id, $change->requested_by, 'approved', [
                'monthly_amount' => $data['monthly_amount'] ?? 0,
                'reviewer_name' => $GLOBALS['wo']['user']['name'] ?? 'Admin'
            ]);
    ...
    // STEP 18: Deny pending change (admin only)
    if ($s === 'deny_pending_change') {
        ...
        $change_id = isset($_POST['change_id']) ? (int)$_POST['change_id'] : 0;
        $denial_reason = isset($_POST['denial_reason']) ? trim($_POST['denial_reason']) : 'Denied by admin';
        ...
        try {
            ...
            // Notify requester
            sendRescheduleNotification($change->purchase_id, $change->requested_by, 'denied', [
                'denial_reason' => $denial_reason
            ]);
    ...
    // STEP 19: Get purchase pending status (for badge counter)
    if ($s === 'get_purchase_pending_status') {
        ...
        try {
            ...
            $pending_count = $db->where('purchase_id', $purchase_id)->where('status', 'pending')->getValue('crm_pending_changes', 'COUNT(*) as cnt');
            
            echo json_encode([
                'status' => 200,
                'purchase_id' => $purchase_id,
                'pending_count' => intval($pending_count ?? 0)
            ]);
        } catch (Exception $e) {
            echo json_encode(['status' => 500, 'message' => 'Error: ' . $e->getMessage()]);
        }
    ...
    // STEP 20: Execute approved reschedule (core logic)
    function executeApprovedReschedule($purchase_id, $monthly_amount, $adjustment_mode, $reason = '') {
        global $db;
        
        try {
            $helper = $db->where('id', $purchase_id)->getOne(T_BOOKING_HELPER);
            if (!$helper) throw new Exception('Purchase not found');
            
            $schedule = $db->where('purchase_id', $purchase_id)->orderBy('installment', 'ASC')->get('crm_payment_schedule');
            if (empty($schedule)) throw new Exception('Schedule not found');
            ...
            foreach ($schedule as $i => $row) {
                if ($row->status == 1) {
                    $last_paid_index = $i;
                    $last_paid_date = $row->payment_date ?: date('Y-m-d');
                }
            }
            
            // Calculate remaining balance from unpaid rows
            $remaining_balance = 0;
            for ($i = $last_paid_index + 1; $i < count($schedule); $i++) {
                $remaining_balance += intval($schedule[$i]->installment_amount ?? 0);
            }
            
            // Archive old schedule (mark as rescheduled)
            for ($i = $last_paid_index + 1; $i < count($schedule); $i++) {
                $db->where('id', $schedule[$i]->id)->update('crm_payment_schedule', [
                    'is_rescheduled' => 1,
                    'remarks' => 'Replaced by reschedule (reason: ' . $reason . ')'
                ]);
            }
            ...
            $reschedule_data = [
                ...
                'adjustment_mode' => $adjustment_mode,
                'reason' => $reason,
                'last_paid_installment_number' => $last_paid_index + 1,
                'affected_rows_count' => max(0, count($schedule) - $last_paid_index - 1),
                'remaining_balance_before' => $remaining_balance,
                'remaining_balance_after' => $remaining_balance,
                'created_by' => $GLOBALS['wo']['user']['id'] ?? null,
                'created_at' => date('Y-m-d H:i:s')
            ];
            
            $reschedule_id = $db->insert('crm_payment_reschedule_history', $reschedule_data);
            
            // Create new schedule rows
            $current_date = new DateTime($last_paid_date);
            $current_date->modify('+1 month');
            $installment_number = $last_paid_index + 2;
            $total_new_installments = ceil($remaining_balance / $monthly_amount);
            
            for ($i = 0; $i < $total_new_installments; $i++) {
                $amount = intval($monthly_amount);
                
                // Last installment handles remainder
                if ($i == $total_new_installments - 1) {
                    $current_sum = $i * intval($monthly_amount);
                    $amount = max(0, $remaining_balance - $current_sum);
                }
                ...
                $new_row = [
                    ...
                    'particular' => 'Rescheduled Installment ' . ($i + 1),
                    'installment_amount' => $amount,
                    'paid_amount' => 0,
                    'date' => $current_date->format('Y-m-d'),
                    'payment_date' => null,
                    'status' => 0,
                    'remarks' => 'New schedule from reschedule',
                    'is_rescheduled' => 1,
                    'reschedule_id' => $reschedule_id,
                    'created_at' => date('Y-m-d H:i:s')
                ];
                
                $db->insert('crm_payment_schedule', $new_row);
                $current_date->modify('+1 month');
            }
            
            return $reschedule_id;
        } catch (Exception $e) {
            error_log('Error in executeApprovedReschedule: ' . $e->getMessage());
            throw $e;
        }
    }

    // STEP 17: Approve pending change
    if ($s === 'approve_pending_change') {
        header('Content-Type: application/json; charset=utf-8');
    ...
    // STEP 19: Get purchase pending status (for badge)
    if ($s === 'get_purchase_pending_status') {
        header('Content-Type: application/json; charset=utf-8');
