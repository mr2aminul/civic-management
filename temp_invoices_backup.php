...
if ($s === 'create_invoice') {
    try {
        ...
        if ($id) {
            // Log to audit trail
            logAuditTrail($purchase_id, 'create', 'invoice', "Invoice $invoice_number created for αº│$amount", null, $data);
...
if ($s === 'record_payment') {
    try {
        ...
        if ($remaining > 0.01) {
            ...
            logAuditTrail($purchase_id, 'create', 'credit', "Overpayment credit created for αº│$remaining", null, $credit_data);
        }

        // Log payment to audit trail
        logAuditTrail($purchase_id, 'create', 'payment', "Payment of αº│$amount recorded via $payment_method", null, $receipt_data);
