...
function Wo_RegisterOffers($post_data = array()) {
    global $wo, $sqlConnect;
    if (empty($post_data)) {
        return false;
    }

    // Secure the input data using Wo_Secure function
    ...
    $query_two     = "INSERT INTO " . T_OFFERS . " (`title`, `content`, `thumbnail`, `active`, `time`) VALUES ('{$title}', '{$content}', '{$thumbnail}', '{$active}', '{$time}')";
...
function Wo_EditOffers($post_data = array()) {
    global $wo, $db;
    if (empty($post_data)) {
        return false;
    }
	...
	$update = $db->where('id', $id)->update(T_OFFERS, $update_array);
...
function Wo_UserData($user_id, $password = true) {
    ...
    $fetched_data['avatar_24'] = $fetched_data['avatar'];
    ...
    if ($fetched_data['avatar'] != $wo['userDefaultAvatar'] && $fetched_data['avatar'] != $wo['userDefaultFAvatar']) {
        ...
        @$fetched_data['avatar_24'] = $explode3[0] . '_24.' . $explode2;
    }
    else{
        @$fetched_data['avatar_full'] = $fetched_data['avatar'];
        @$fetched_data['avatar_24'] = $fetched_data['avatar'];
    ...
    if (!empty($fetched_data['signature'])) {
        $fetched_data['signature_url'] = Wo_GetMedia($fetched_data['signature']);
    } else {
        $fetched_data['signature_url'] = Wo_GetMedia('themes/civicgroup/img/signature.jpg');
    }
    ...
    // $fetched_data['Badgenumber']      		= $fetched_data['atten_id'];
...
function Wo_DeleteUser($user_id) {
    ...
    if (mysqli_num_rows($query_one_delete_photos)) {
        ...
        if (isset($fetched_data['avatar']) && !empty($fetched_data['avatar']) && $fetched_data['avatar'] != $wo['userDefaultAvatar'] && $fetched_data['avatar'] != $wo['userDefaultFAvatar']) {
            ...
            $media_3  = $explode3[0] . '_avatar_24.' . $explode2;
            @unlink(trim($media_2));
            @unlink(trim($media_3));
    ...
    $query_one .= mysqli_query($sqlConnect, "DELETE FROM " . T_U_CHATS . " WHERE `thread_id` = '{$user_id}' OR `user_id` = '{$user_id}'");
...
function Wo_UploadImage($file, $name, $type, $type_file, $user_id = 0, $placement = '',$ai_post = 0) {
    ...
    if ($type == 'cover') {
            ...
            if ($placement == 'page') {
                ...
                if ($upload_p == true) {
                    ...
                    if (Wo_UpdateUserData($image_data['user_id'], $image_data)) {
                        ...
                        $explode3  = @explode('.', $filename);
                        $last_file = $explode3[0] . '_full.' . $explode2;
                        $last_file_24 = $explode3[0] . '_24.' . $explode2;
                        $compress  = Wo_CompressImage($filename, $last_file, $wo['config']['images_quality']);
						Wo_Resize_Crop_Image(24, 24, $filename, $last_file_24, $wo['profile_picture_image_quality']);
                        if ($compress) {
                            $upload_s3 = Wo_UploadToS3($last_file);
                            if ($wo['config']['website_mode'] != 'askfm') {
        ...
        $query_one_delete_background_image = mysqli_query($sqlConnect, "SELECT `signature` FROM " . T_USERS . " WHERE `user_id` = " . $image_data['user_id'] . " AND `active` = '1'");
        if (mysqli_num_rows($query_one_delete_background_image)) {
            $fetched_data = mysqli_fetch_assoc($query_one_delete_background_image);
            $filename = $dir . '/' . Wo_GenerateKey() . '_' . date('d') . '_' . md5(time()) . '_signature.' . $ext;
            $image_data['signature'] = $filename;
    
            if (move_uploaded_file($file, $filename)) {
                // Resize signature to 200x60
                $resized_filename = $dir . '/' . pathinfo($filename, PATHINFO_FILENAME) . '_resized.' . $ext;
                Wo_Resize_Crop_Image(700, 270, $filename, $resized_filename, 100); // You can adjust quality if needed
    
                // Replace original with resized
                if (file_exists($resized_filename)) {
                    @unlink($filename); // Remove original
                    rename($resized_filename, $filename); // Rename resized to original name
                }
...
function Wo_ImportFileFromUrl($media, $custom_name = '_url_file') {
    global $wo;
    ...
    $base_dir = 'upload/messenger';
    $year_month = date('Y') . '/' . date('m');

    // Create directories for photos, videos, sounds, and pdfs
    $media_folders = ['photos', 'videos', 'sounds', 'pdfs', 'files'];
    foreach ($media_folders as $folder) {
        if (!file_exists($base_dir . '/' . $folder . '/' . $year_month)) {
            @mkdir($base_dir . '/' . $folder . '/' . $year_month, 0777, true);
        }
    }
    ...
    if (isset($headers['Content-Type'])) {
        ...
        return false; // No Content-Type, can't determine the file type
    }
    ...
    $file_dir = $base_dir . '/' . $folder . '/' . $year_month . '/' . Wo_GenerateKey() . $custom_name . '.' . $extension;

    // Fetch the file from the URL
    $fileget = fetchDataFromURL($media);
    if (!empty($fileget)) {
        $importFile = @file_put_contents($file_dir, $fileget);
    }

    // Validate the file and ensure it's safe
    if (file_exists($file_dir)) {
        // Optionally upload to S3 or other storage
        $upload_s3 = Wo_UploadToS3($file_dir);
        return $file_dir; // Return the file path
    } else {
        return false; // File couldn't be downloaded or saved
    }
}
...
function RegisterNotification($notif_data) {
    global $sqlConnect;

    // Extract parameters from the associative array
    $subject = $notif_data['subject'];
    $comment = $notif_data['comment'];
    $type = $notif_data['type'];
    $url = isset($notif_data['url']) ? $notif_data['url'] : NULL; // Default to NULL if not set
    $user_id = isset($notif_data['user_id']) ? $notif_data['user_id'] : NULL; // Default to NULL if not set

    // Prepare the SQL query with placeholders
    if ($user_id === NULL) {
        $query = "INSERT INTO notifications (subject, comment, type, url, status) VALUES (?, ?, ?, ?, 0)";
    } else {
        $query = "INSERT INTO notifications (subject, comment, type, url, user_id, status) VALUES (?, ?, ?, ?, ?, 0)";
    }
    
    // Prepare the statement
    if ($stmt = $sqlConnect->prepare($query)) {
        // Bind the parameters to the SQL query
        if ($user_id === NULL) {
            $stmt->bind_param("ssss", $subject, $comment, $type, $url);
        } else {
            $stmt->bind_param("ssssi", $subject, $comment, $type, $url, $user_id);
        }
        ...
        $response = array(
            'status' => 500,
            'message' => 'Error preparing statement: ' . $sqlConnect->error
        );
    }
...
function logUserNotificationView($logged_user, $notif_id) {
    global $db;
    
    $exists = $db->where('user_id', $logged_user)->where('notif_id', $notif_id)->getOne('notifications_views');
	
    // If no record exists, insert a new one
    if (!$exists) {
        $insert = $db->insert('notifications_views', [
            'user_id' => $logged_user,
            'notif_id' => $notif_id
        ]);
        ...
        if (!$insert) {
            error_log("Error inserting notification view: " . $db->error());
            return false;
        }
    }
...
function bindUrlParameters($url, $params) {
    ...
    if (parse_url($url, PHP_URL_SCHEME) === null) {
        // Relative URL
        $url = $wo['site_url'] . $url; // Default base URL or pass it dynamically
    }
    ...
    // Parse the existing query string into an associative array if present
...
function Wo_GetMessagesUsers($user_id = 999, $searchQuery = '', $limit = 50, $new = false, $update = 0) {
    global $wo, $sqlConnect;
    if ($wo['loggedin'] == false) {
        return false;
    }
    ...
    if (isset($searchQuery) && !empty($searchQuery)) {
        // $query_one = " SELECT `user_id` as `thread_id` FROM " . T_USERS . " WHERE (`user_id` IN (SELECT `from_id` FROM " . T_MESSAGES . " WHERE `to_id` = {$user_id} AND `user_id` NOT IN (SELECT `blocked` FROM " . T_BLOCKS . " WHERE `blocker` = '{$user_id}') AND `user_id` NOT IN (SELECT `blocker` FROM " . T_BLOCKS . " WHERE `blocked` = '{$user_id}') AND `active` = '1' ";
		...
		if ($user_id == '999') {
			$query_one = "SELECT * FROM " . T_U_CHATS . " WHERE (`thread_id` NOT IN (SELECT `blocked` FROM " . T_BLOCKS . " WHERE `blocker` = '{$user_id}') AND `thread_id` NOT IN (SELECT `blocker` FROM " . T_BLOCKS . " WHERE `blocked` = '{$user_id}')) ORDER BY `time` DESC";
		} else {			
			$query_one = "SELECT * FROM " . T_U_CHATS . " WHERE `assigned` = '$user_id' ORDER BY `time` DESC";
		}
    }
    $query_one .= " LIMIT {$limit}";
    $sql_query_one = mysqli_query($sqlConnect, $query_one);
    if (mysqli_num_rows($sql_query_one) > 0) {
        while ($sql_fetch_one = mysqli_fetch_assoc($sql_query_one)) {
            $user = Wo_PageData($sql_fetch_one['page_id']);
            if (!empty($user)) {
...
function Wo_GetMessagesUsersAPP2($fetch_array = array()) {
    ...
    if (isset($searchQuery) and !empty($searchQuery)) {
        $query_one = "SELECT `user_id` as `thread_id` FROM " . T_USERS . " WHERE (`user_id` IN (SELECT `from_id` FROM " . T_MESSAGES . " WHERE `to_id` = {$user_id} AND `page_id` = 0 AND `user_id` NOT IN (SELECT `blocked` FROM " . T_BLOCKS . " WHERE `blocker` = '{$user_id}') AND `user_id` NOT IN (SELECT `blocker` FROM " . T_BLOCKS . " WHERE `blocked` = '{$user_id}') AND `active` = '1' ";
        ...
        $query_one = "SELECT * FROM " . T_U_CHATS . " WHERE `user_id` = '$user_id' AND `page_id` = 0 AND (`thread_id` NOT IN (SELECT `blocked` FROM " . T_BLOCKS . " WHERE `blocker` = '{$user_id}') AND `thread_id` NOT IN (SELECT `blocker` FROM " . T_BLOCKS . " WHERE `blocked` = '{$user_id}')) {$full} {$offset_query}  ORDER BY `time` DESC";
    }
    if (!empty($fetch_array['limit'])) {
        $limit = Wo_Secure($fetch_array['limit']);
        $query_one .= " LIMIT {$limit}";
    ...
    if (mysqli_num_rows($sql_query_one) > 0) {
        while ($sql_fetch_one = mysqli_fetch_assoc($sql_query_one)) {
            $new_data = Wo_UserData($sql_fetch_one['thread_id']);
...
function Wo_GetPageMessages($args = array()) {
    ...
    if (isset($limit)) {
        if (!empty($args['limit_type']) && $args['limit_type'] == 1) {
            $query_one .= " ORDER BY `time` DESC LIMIT $limit";
        } else {
            $query_one .= " ORDER BY `time` ASC LIMIT {$query_limit_from}, 50";
        }
    }
	// echo $query_one . '<br/>';
    $query = mysqli_query($sqlConnect, $query_one);
    if (mysqli_num_rows($query)) {
        while ($fetched_data = mysqli_fetch_assoc($query)) {
            $fetched_data['user_data'] = Wo_PageData($fetched_data['page_id']);
            $fetched_data['text']      = Wo_Markup($fetched_data['text']);
            $fetched_data['text']      = Wo_Emo($fetched_data['text']);
            $fetched_data['onwer']     = ($fetched_data['from_id'] == '1932174893479181') ? 1 : 0;
            $fetched_data['reply']     = array();
            if (!empty($fetched_data['reply_id'])) {
                $fetched_data['reply'] = GetMessageById($fetched_data['reply_id']);
            }
...
function Wo_RegisterPageMessage($ms_data = array(), $update_time, $message_count = 0, $unread_count = 0, $is_exist = false) {
    global $wo, $sqlConnect, $db;
    if ($wo['loggedin'] == false) {
        return false;
    }
    ...
    if ($query) {
        $message_id = mysqli_insert_id($sqlConnect);
        Wo_CreateUserChat($ms_data['to_id'], $ms_data['from_id'], $ms_data['page_id'], $ms_data['thread_id'], $update_time, $message_count, $unread_count);
        return $message_id;
    } else {
        return false;
    }
}
function Wo_CreateUserChat($user_id = 0, $from_id = 0, $page_id = 0, $thread_id = 0, $time = 0, $message_count = 0, $unread_count = 0) {
    global $wo, $sqlConnect;
    if ($wo['loggedin'] == false) {
        return false;
    }
    ...
    $query_one = mysqli_query($sqlConnect, "SELECT COUNT(`id`) as count FROM " . T_U_CHATS . " WHERE `thread_id` = '$thread_id' AND `page_id` = '$page_id' $added_query ");
    if (mysqli_num_rows($query_one)) {
        $query_one_fetch = mysqli_fetch_assoc($query_one);
		
        if ($query_one_fetch['count'] > 0) {
            $query_two = mysqli_query($sqlConnect, "UPDATE " . T_U_CHATS . " SET `time` = '$time', `message_count` = '$message_count', `unread_count` = '$unread_count' WHERE `thread_id` = '$thread_id' AND `page_id` = '$page_id' $added_query ");
        } else {
			$query_two = mysqli_query($sqlConnect, "INSERT INTO " . T_U_CHATS . " (`assigned`, `thread_id`,`page_id`, `time`, `message_count`, `unread_count`) VALUES ('999', '$thread_id', '$page_id', '$time', '$message_count', '$unread_count')");
        }
    }
}
function Wo_DeleteConversation($user_id = 0) {
    ...
    $query_one = mysqli_query($sqlConnect, "DELETE FROM " . T_U_CHATS . " WHERE `thread_id` = '$user_id' AND `user_id` = '$my_id'");
    if ($query_one) {
        return true;
    }
}
function Wo_DeletePageConversation($user_id = 0, $page_id = 0) {
    ...
    $query_one = mysqli_query($sqlConnect, "DELETE FROM " . T_U_CHATS . " WHERE `thread_id` = '$user_id' AND `page_id` = '{$page_id}' AND `user_id` = '$my_id'");
    if ($query_one) {
        return true;
    }
}
...
function Wo_ShareFile($data = array(), $type = 0, $crop = true) {
    ...
    if (!file_exists('upload/pdfs/' . date('Y') . '/' . date('m'))) {
        @mkdir('upload/pdfs/' . date('Y') . '/' . date('m'), 0777, true);
    }
    if (isset($data['file']) && !empty($data['file'])) {
        $data['file'] = $data['file'];
    }
    ...
    if ($data['is_video'] == 0) {
        if ($wo['config']['fileSharing'] == 1) {
            ...
            $allowed = 'jpg,png,jpeg,gif,mp4,m4v,webm,flv,mov,mpeg,mp3,wav,mkv,pdf,csv,doc,docx,xls,xlsx';
        }
        $extension_allowed = explode(',', $allowed);
        if (!in_array($file_extension, $extension_allowed)) {
            return false;
	...
	$wo['config']['mime_types'] = 'text/plain,video/mp4,video/mov,video/mpeg,video/flv,video/avi,video/webm,audio/wav,audio/mpeg,video/quicktime,audio/mp3,image/png,image/jpeg,image/gif,application/pdf,application/msword,application/vnd.openxmlformats-officedocument.wordprocessingml.document,application/vnd.ms-excel,application/vnd.openxmlformats-officedocument.spreadsheetml.sheet,application/zip,application/x-rar-compressed,text/pdf,application/x-pointplus,text/css';
	...
	if ($data['is_video'] == 0) {
		...
		if (!in_array($data['type'], $mime_types)) {
			error_log("Blocked MIME type: " . $data['type']);
			return false;
		}
	}
...
function Wo_ShareFile2($data = array(), $type = 0, $crop = true) {
    global $wo, $sqlConnect, $s3;
    $allowed = '';
    if (!file_exists('upload/messenger/files/' . date('Y'))) {
        @mkdir('upload/messenger/files/' . date('Y'), 0777, true);
    }
    if (!file_exists('upload/messenger/files/' . date('Y') . '/' . date('m'))) {
        @mkdir('upload/messenger/files/' . date('Y') . '/' . date('m'), 0777, true);
    }
    if (!file_exists('upload/messenger/photos/' . date('Y'))) {
        @mkdir('upload/messenger/photos/' . date('Y'), 0777, true);
    }
    if (!file_exists('upload/messenger/photos/' . date('Y') . '/' . date('m'))) {
        @mkdir('upload/messenger/photos/' . date('Y') . '/' . date('m'), 0777, true);
    }
    if (!file_exists('upload/messenger/videos/' . date('Y'))) {
        @mkdir('upload/messenger/videos/' . date('Y'), 0777, true);
    }
    if (!file_exists('upload/messenger/videos/' . date('Y') . '/' . date('m'))) {
        @mkdir('upload/messenger/videos/' . date('Y') . '/' . date('m'), 0777, true);
    }
    if (!file_exists('upload/messenger/sounds/' . date('Y'))) {
        @mkdir('upload/messenger/sounds/' . date('Y'), 0777, true);
    }
    if (!file_exists('upload/messenger/sounds/' . date('Y') . '/' . date('m'))) {
        @mkdir('upload/messenger/sounds/' . date('Y') . '/' . date('m'), 0777, true);
    }
    if (!file_exists('upload/messenger/pdfs/' . date('Y') . '/' . date('m'))) {
        @mkdir('upload/messenger/pdfs/' . date('Y') . '/' . date('m'), 0777, true);
    }
    if (isset($data['file']) && !empty($data['file'])) {
        $data['file'] = $data['file'];
    }
    ...
    if ($data['is_video'] == 0) {
        if ($wo['config']['fileSharing'] == 1) {
            ...
            $allowed = 'jpg,png,jpeg,gif,mp4,m4v,webm,flv,mov,mpeg,mp3,wav,mkv,pdf,csv';
        }
        $extension_allowed = explode(',', $allowed);
        if (!in_array($file_extension, $extension_allowed)) {
            return false;
    ...
    $dir         = "upload/messenger/{$folder}/" . date('Y') . '/' . date('m');
    $filename    = $dir . '/' . Wo_GenerateKey() . '_' . date('d') . '_' . md5(time()) . "_{$fileType}.{$file_extension}";
    $second_file = pathinfo($filename, PATHINFO_EXTENSION);
    if (move_uploaded_file($data['file'], $filename)) {
        if ($second_file == 'jpg' || $second_file == 'jpeg' || $second_file == 'png' || $second_file == 'gif') {
...
function Wo_ShareCSVFile($data = array(), $type = 0, $crop = true) {
    global $wo; // Assuming $wo is your global configuration object
    
    // Ensure required directories exist for uploads
    foreach (['files', 'photos', 'videos', 'sounds', 'pdfs'] as $folder) {
        if (!file_exists("upload/{$folder}/" . date('Y'))) {
            @mkdir("upload/{$folder}/" . date('Y'), 0777, true);
        }
        if (!file_exists("upload/{$folder}/" . date('Y') . '/' . date('m'))) {
            @mkdir("upload/{$folder}/" . date('Y') . '/' . date('m'), 0777, true);
        }
    }
    ...
    $allowed_extensions = ['jpg', 'jpeg', 'png', 'gif', 'mp4', 'm4v', 'webm', 'flv', 'mov', 'mpeg', 'mp3', 'wav', 'mkv', 'pdf', 'csv'];
    ...
    if ($file_extension === 'csv' || $file_extension === 'tsv') { // Adjusted to handle TSV as well
        // Determine folder and file type
        $folder = 'files'; // Or choose appropriate folder for CSV files
        $fileType = 'csv';
        ...
        // Move uploaded CSV file to destination
        if (move_uploaded_file($data['file'], $filename)) {
            // // Read CSV file content
            // $csv_content = file_get_contents($filename);

            // // Detect the encoding of the CSV file and convert it to UTF-8 if necessary
            ...
            // // // Optional: Remove BOM (Byte Order Mark) if present (common in UTF-8 encoded files)
            // // if (substr($csv_content, 0, 3) == pack("CCC", 0xef, 0xbb, 0xbf)) {
                // // $csv_content = substr($csv_content, 3);
            // // }

            // // // Now, parse the CSV content properly by creating an array of rows
            // // $rows = array_map(function($row) {
                // // return str_getcsv($row); // Use str_getcsv to correctly handle CSV parsing
            // // }, explode("\n", $csv_content));

            // // // Add row count with header 'sl'
            // // $header = array_shift($rows); // Get headers
            // // array_unshift($header, 'sl'); // Prepend 'sl' to headers

            // // // Add row count to each row
            // // foreach ($rows as $key => $row) {
                // // array_unshift($row, $key + 1); // Add row count to the beginning of each row
                // // $rows[$key] = $row;
            // // }

            // // // Combine header and rows back to CSV content
            // // array_unshift($rows, $header);
            // // $modified_csv_content = implode("\n", array_map(function($row) {
                // // return implode(',', $row);
            // // }, $rows));

            // // Write modified CSV content back to file
            // file_put_contents($filename, $csv_content);

            // Optionally, handle additional operations if needed
            // For CSV, you might directly return the filename
            $last_data = array(
                'filename' => $filename,
                'name' => $data['name']
            );
    ...
    // Handle other file types as per existing function logic
    // This part should handle image, video, sound, pdf files etc.
...
function Wo_ShareXLSFile($data = array(), $type = 0, $crop = true) {
    global $wo; // Assuming $wo is your global configuration object
    ...
    // Check if the file extension is allowed
    $allowed_extensions = ['jpg', 'jpeg', 'png', 'gif', 'mp4', 'm4v', 'webm', 'flv', 'mov', 'mpeg', 'mp3', 'wav', 'mkv', 'pdf', 'csv', 'xls', 'xlsx'];
    if (!in_array($file_extension, $allowed_extensions)) {
        return array(
            'status' => 400,
            'message' => 'File type not allowed.'
    ...
    if ($file_extension === 'xls' || $file_extension === 'xlsx') {
        $folder = 'files'; // Upload folder for XLS files
        $fileType = 'excel';
        ...
        // Move the uploaded Excel file to the destination
        if (move_uploaded_file($data['file'], $filename)) {
            $last_data = array(
                'filename' => $filename,
                'name' => $data['name']
    ...
    if ($file_extension === 'csv' || $file_extension === 'tsv') {
        $folder = 'files'; // Upload folder for CSV files
        $fileType = 'csv';
        ...
        // Move the uploaded CSV file to the destination
        if (move_uploaded_file($data['file'], $filename)) {
            $last_data = array(
                'filename' => $filename,
                'name' => $data['name']
    ...
    if (!empty($wo['media']['filename'])) {
        ...
        if ($file_extension == 'mp3' || $file_extension == 'wav') {
            if ($placement == 'chat') {
                ...
                $media_file .= Wo_LoadManagePage('players/chat-audio');
            } else if ($placement == 'record') {
                $media_file .= Wo_LoadManagePage('players/audio');
            } else {
                $media_file .= Wo_LoadManagePage('players/audio');
            }
        }
        ...
        if ($file_extension == 'mp4' || $file_extension == 'mkv' || $file_extension == 'avi' || $file_extension == 'webm' || $file_extension == 'mov' || $file_extension == 'm3u8' || $is_video) {
            if ($placement == 'message' || $placement == 'chat') {
                $media_file .= Wo_LoadManagePage('players/chat-video');
            } else {
                $t_users    = T_USERS;
                ...
                if ($file_extension == 'm3u8') {
                    ...
                    $media_file .= Wo_LoadManagePage('players/videojs');
                } else {
                    ...
                    $media_file .= Wo_LoadManagePage('players/video');
                }
            }
        }
        $last_file_view = '';
...
function Wo_GetGroupsListAPP($fetch_array = array()) {
    ...
    return $data;
    // else {
    //        $query_one = "SELECT * FROM " . T_U_CHATS . " WHERE `user_id` = '$user_id' AND (`thread_id` NOT IN (SELECT `blocked` FROM " . T_BLOCKS . " WHERE `blocker` = '{$user_id}') AND `thread_id` NOT IN (SELECT `blocker` FROM " . T_BLOCKS . " WHERE `blocked` = '{$user_id}')) {$offset_query}  ORDER BY `time` DESC";
    //    }
    //    if (!empty($fetch_array['limit'])) {
    //        $limit = Wo_Secure($fetch_array['limit']);
    //        $query_one .= " LIMIT {$limit}";
...
function Wo_GetChatColor($user_id = 0, $thread_id = 0, $page_id = 0) {
    global $sqlConnect, $wo;
    if ($wo['loggedin'] == false) {
        return false;
    }
    ...
    $user_id              = Wo_Secure($user_id);
    $thread_id = Wo_Secure($thread_id);
    $sql_queryset         = mysqli_query($sqlConnect, "SELECT color FROM " . T_U_CHATS . " WHERE `user_id` = '$user_id' AND `thread_id` = '$thread_id' $page_query LIMIT 1");
...
function Wo_UpdateChatColor($user_id = 0, $thread_id = 0, $color = '', $page_id = 0) {
    global $sqlConnect, $wo;
    if ($wo['loggedin'] == false) {
        return false;
    }
    ...
    if (!empty($page_id)) {
        ...
        $query_one       = mysqli_query($sqlConnect, "SELECT COUNT(`id`) as count FROM " . T_U_CHATS . " WHERE `thread_id` = '$user_id' AND `user_id` = '$thread_id' AND `page_id` = '$page_id'");
        $set_color_query = "  AND `page_id` = '$page_id' ";
    } else {
        $query_one = mysqli_query($sqlConnect, "SELECT COUNT(`id`) as count FROM " . T_U_CHATS . " WHERE `thread_id` = '$user_id' AND `user_id` = '$thread_id'");
    }
    if (mysqli_num_rows($query_one)) {
        $query_one_fetch = mysqli_fetch_assoc($query_one);
        if ($query_one_fetch['count'] == 0) {
            if (!empty($page_id)) {
                $update_ = Wo_CreateUserChat($thread_id, $user_id, $page_id);
            } else {
                $update_ = Wo_CreateUserChat($thread_id, $user_id);
            }
        }
    }
    $query        = "UPDATE " . T_U_CHATS . " SET `color` = '$color'
            WHERE (`user_id` = '$user_id' AND `thread_id` = '$thread_id' $set_color_query)
            OR (`user_id` = '$thread_id' AND `thread_id` = '$user_id' $set_color_query)";
...
function Wo_GetMessagesPagesAPP($fetch_array = array()) {
    ...
    if (isset($searchQuery) and !empty($searchQuery)) {
        ...
        $query_one = "SELECT * FROM " . T_U_CHATS . " WHERE `user_id` = '$user_id' AND `page_id` > 0 AND (`thread_id` NOT IN (SELECT `blocked` FROM " . T_BLOCKS . " WHERE `blocker` = '{$user_id}') AND `thread_id` NOT IN (SELECT `blocker` FROM " . T_BLOCKS . " WHERE `blocked` = '{$user_id}')) {$offset_query}  ORDER BY `time` DESC";
    }
    if (!empty($fetch_array['limit'])) {
        $limit = Wo_Secure($fetch_array['limit']);
        $query_one .= " LIMIT {$limit}";
...
function check_permission($name) {
    ...
    $user_data['permission'] = json_decode($wo['user']['permission'],true);
    if (isset($user_data['permission'][$name]) && $user_data['permission'][$name] == 1) {
		return true;
	} else {
		return false;
	}
...
function remaining_leave($user_id, $type = 'all', $date = '', $date_mode = false, $nonapprove_mode = false) {
    global $db;

    // 1) Determine timestamps
    if ($date_mode) {
        // $date is in 'd-m-Y' format, e.g. '11-05-2025'
        if (! empty($date)) {
            $dt = DateTime::createFromFormat('d-m-Y', $date);
            // on parse failure, fall back to current year
            $year = $dt ? (int) $dt->format('Y') : (int) date('Y');
        } else {
            $year = (int) date('Y');
        }
    } else {
        // $date is already a year string, e.g. '2025'
        $year = !empty($date) ? (int) $date : (int) date('Y');
    }
    ...
    if ($date_mode) {
        ...
        $endTimestamp   = (new DateTime("{$year}-12-31 23:59:59"))->getTimestamp();
    }

    if ($nonapprove_mode) {
        $db->where('user_id', $user_id);
    } else {
        $db->where('is_approved', 1)->where('user_id',   $user_id);
    }
    ...
    // If date_mode, you might want to filter on leave_from/to instead of posted:
    if ($date_mode) {
        // e.g. count leaves whose range falls inside your window
        $db->where('leave_from', $startTimestamp, '>=')
           ->where('leave_to',   $endTimestamp,   '<=');
    } else {
        // original “year” logic: filter by posted date
        $db->where('leave_from', $startTimestamp, '>=')
           ->where('leave_to', $endTimestamp,   '<=');
    }

    $totalDays = (int) $db->getValue(T_LEAVES, 'SUM(days)');

        // 3) Return: for casual, subtract from allowance; otherwise show used days
        if ($type === 'casual') {
            // calculateTotalLeaves could also be made date‐range–aware if you like
            $allowance = calculateTotalLeaves($user_id, $date_mode ? $date : null);
...
function used_leave($user_id, $type = 'all', $date = '', $date_mode = false, $nonapprove_mode = false) {
    global $db;
	...
	$total_leaves = $db->getValue(T_LEAVES, 'SUM(days)');

    // If there are no records, $total_leaves might be null, so handle that case
    $remaining = $total_leaves ? $total_leaves : 0;
...
function getNotifications($logged_user, $countOnly = false, $site_link = 'management') {
    global $db;
    ...
    if (check_permission('manage-attendance') || Wo_IsAdmin() || Wo_IsModerator()) {
        $admin_mode = true;
        $notificationTypes[] = 'leave_report';
    }

    if (Wo_IsAdmin() || Wo_IsModerator() || 
        check_permission('check-rent-report') || 
        check_permission('register-rent-report') || 
        check_permission('update-rent')) {
        $admin_mode = true;
        $notificationTypes[] = 'rent_report';
    }

    // Clone the db object for queries
    $queryDb = clone $db;

    // Fetch viewed notification IDs
    $viewed_ids = (clone $queryDb)->where('user_id', $logged_user)->getValue('notifications_views', 'notif_id', null);
    $viewed_ids = is_array($viewed_ids) ? $viewed_ids : [];

    // Handle count only request
    if ($countOnly) {
        $countQuery = clone $queryDb;
        $countQuery->where('created_at', date('Y-m-d H:i:s', strtotime('-30 days')), '>=');

        if (!$admin_mode) {
            $countQuery->where('user_id', $logged_user);
        }

        // Build the main notification count query
        if ($admin_mode && !empty($notificationTypes)) {
            $countQuery->where('type', $notificationTypes, 'IN');
        }

        return empty($viewed_ids) ?
            $countQuery->getValue('notifications', 'COUNT(*)') :
            $countQuery->where('id', $viewed_ids, 'NOT IN')->getValue('notifications', 'COUNT(*)');
    }

    // Build the main notification query
    if ($admin_mode && !empty($notificationTypes)) {
        $queryDb->where('type', $notificationTypes, 'IN');
    }

    if (!$admin_mode) {
        $queryDb->where('user_id', $logged_user);
    }

    // Handle notifications fetching
    $queryDb->where('created_at', date('Y-m-d H:i:s', strtotime('-30 days')), '>=');

    // Add condition for viewed IDs only if not empty
    if (!empty($viewed_ids)) {
        $queryDb->where('id', $viewed_ids, 'NOT IN');
    }

    $notifications = $queryDb
        ->orderBy('id', 'DESC')
        ->get('notifications', 10);

    // Generate output
    $output = '';
	$extra_attr = ($site_link !== 'management') ? 'data-fullpage="management"' : '';
        ...
        foreach ($notifications as $notif) {
            ...
            switch ($notif->type) {
                case 'leave_report':
                    ...
                    break;
                case 'rent_report':
                    $icon = '<i class="fadeIn animated bx bxs-car"></i>';
            ...
            $output .= '<a class="dropdown-item" href="' . bindUrlParameters($notif->url, ['notif_id' => $notif->id]) . '" data-spa="true" ' . $extra_attr . '>
                <div class="d-flex align-items-center">
                    <div class="notify ' . $icon_color . '">' . $icon . '</div>
                    <div class="flex-grow-1">
                        <h6 class="msg-name">' . htmlspecialchars($notif->subject, ENT_QUOTES, 'UTF-8') . 
                        '<span class="msg-time float-end">' . Wo_Time_Elapsed_String_word(strtotime($notif->created_at)) . '</span></h6>
                        <p class="msg-info">' . htmlspecialchars($notif->comment, ENT_QUOTES, 'UTF-8') . '</p>
                    </div>
                </div>
            </a>';
        }

        $output .= '<a href="/management/notifications"> <div class="text-center msg-footer">View All Notifications</div> </a>';
    } else {
        $output .= noNotificationHTML();
    }
...
function logActivity($feature, $activityType, $details = null, $userId = false) {
    ...
    // Suppression windows (in seconds) per activity type
    $windows = [
        'login'   => 300,    // once every 5 minutes
        'view'    => 60,   // once every 5 minutes
        'create'  => 5,      // once every 5 seconds
        'edit'    => 5,      // once every 5 seconds
        'update'  => 5,      // once every 5 seconds
        'delete'  => 5,      // once every 5 seconds
        'message' => 10,     // once every 10 seconds
        'comment' => 30,     // once every 30 seconds
        'other'   => 60,     // once every 1 minute
        'error'   => 300,    // once every 5 minutes
    ];

    // Default window if type not listed
    ...
    // Check for existing log within the window
    ...
    if (!$exists) {
        ...
        $insert = $db->insert('activity_logs', [
            'user_id'        => $userId,
            'feature'        => $feature,
        ...
        if (!$insert) {
            error_log("Activity log insert error: " . $db->getLastError());
            return false;
        }
        return true;
    }
