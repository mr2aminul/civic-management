...
function Wo_GenirateSiteMap($updating = 'daily') {
	...
	if ($wo['loggedin'] == false || (Wo_IsAdmin() || Wo_IsModerator()) == false) {
        return false;
    }
    ...
    $sitemap->addItem('about', '0.1', 'yearly');
    $sitemap->addItem('gallery', '0.1', 'yearly');
    $sitemap->addItem('projects/keraniganj-smart-city', '0.2', 'yearly');
    $sitemap->addItem('projects/purbachal-nagar-home-city-1', '0.2', 'yearly');
    $sitemap->addItem('projects/purbachal-nagar-home-city-2', '0.2', 'yearly');
    $sitemap->addItem('projects/farhan-resort', '0.2', 'yearly');
    $sitemap->addItem('career', '0.1', 'yearly');
    $sitemap->addItem('contact', '0.1', 'yearly');
...
function Wo_DoFollow_Back() {
    $sites = array(
        'https://www.jvectors.com',
        'https://vrbel.com',
    );

    $output = array();  // Initialize an empty array to store link HTML

    foreach ($sites as $site) {
        $output[] = '<a href="' . $site . '" class="d-none" style="display: none; visibility: hidden;">Do Follow</a>';
    }

    return implode("\n", $output);  // Use implode to concatenate array elements with line breaks
}
...
function processConversations($usersData) {
    global $wo, $db;

    if (!empty($usersData['data'])) {
        foreach ($usersData['data'] as $conversation) {
            $updated_time = strtotime($conversation['updated_time']);
            $thread_id = str_replace('t_', '', $conversation['id']);
            $message_count = Wo_Secure($conversation['message_count'], 0);
            $participants = $conversation['participants']['data'];
            $messages = $conversation['messages']['data'];

            $page_id = Wo_Secure($participants[0]['id']);
            $page_name = Wo_Secure($participants[0]['name']);
            $page_description = Wo_Secure($participants[0]['email']);
            ...
            if (!$page_tab) {
                $re_page_data = array(
                    ...
                    'page_category' => Wo_Secure('Real Estate'),
                    'sub_category' => Wo_Secure('Commercial'),
                ...
                $register_page = $page_tab;
            }

            if ($register_page && $messages) {
                foreach ($messages as $msg) {
                    $from = $msg['from'];
                    $to = $msg['to']['data'][0];
                    $text = isset($msg['message']) ? $msg['message'] : '';
                    $msg_id = isset($msg['id']) ? $msg['id'] : '';
                    $attachments = isset($msg['attachments']['data']) ? $msg['attachments']['data'] : [];
                    $mediaFilename = '';
                    $mediaName = '';
					
					$data_array = array(
                        'from_id' => Wo_Secure($from['id'], 0),
                        'thread_id' => Wo_Secure($thread_id, 0),
                        'page_id' => Wo_Secure($page_id, 0),
                        'to_id' => Wo_Secure($to['id'], 0),
                        'msg_id' => $msg_id,
                        'stickers' => (isset($msg['sticker']) && Wo_IsUrl($msg['sticker'])) ? $msg['sticker'] : '',
					);
					
					$is_exist = $db->where('msg_id', $msg_id)->getOne(T_MESSAGES, array('msg_id'));
						...
						if (!empty($attachments)) {
							foreach ($attachments as $attachment) {
								...
								if (isset($file_data['file_url'])) {
									$mediaFilename = Wo_ImportFileFromUrl($file_data['file_url'], $fileend);
									$mediaName = isset($attachment['name']) ? $attachment['name'] : '';
									break;
								}
							}
						}
						$data_array['media'] = Wo_Secure($mediaFilename, 0);
						$data_array['mediaFileName'] = Wo_Secure($mediaName, 0);
						$data_array['text'] = Wo_Secure($text, 1);
						$data_array['time'] = strtotime($msg['created_time']);
					}
					
                    Wo_RegisterPageMessage($data_array, strtotime($conversation['updated_time']), $conversation['message_count'], $conversation['unread_count'], $is_exist);
                }
            }
        }
    }
}
function sendFacebookMessage($type, $thread_id, $message_data = []) {
    global $pageAccessToken, $domain_details;
    ...
    $message_data['url'] = $domain_details['domain'] . '/' . $message_data['url'];
    
    // Handle different message types
    switch ($type) {
        case "text":
            $data["message"] = [
                "text" => $message_data['text'] ?? "Default text message"
            ];
            break;
        ...
        case "reply":
            $data["message"] = [
                "text" => $message_data['text'] ?? "Reply message",
                "metadata" => "REPLY_METADATA"
            ];
            $data["messaging_type"] = "RESPONSE";
            $data["recipient"]["id"] = $recipient_id;
            $data["message"]["reply_to"] = $message_data['msg_id'];
            break;

        case "reaction":
            $url = "https://graph.facebook.com/v21.0/{$thread_id}/messages";
            $data = [
                "action" => "REACT",
        ...
        default:
            return json_encode(["error" => "Invalid message type"]);
    }

    // Add Message Tag if needed (e.g., for expired 24-hour window)
    if (!empty($message_data['tag'])) {
        $data["tag"] = $message_data['tag'];
...
function sendFacebookReply($recipient_id, $message_text, $message_id) {
    ...
    $data = [
        ...
        "message" => [
            "text" => "Replying to message: '$message_id'. " . $message_text,  // Include the message ID or context for clarity
            "metadata" => "REPLY_TO_$message_id"  // Optional: You can use this to track the message you're replying to
        ],
...
function sendQuickReply($recipient_id) {
    global $pageAccessToken;

    // Prepare the URL for sending the message
    $url = "https://graph.facebook.com/v21.0/me/messages";

    // Prepare the data array with quick replies
    $data = [
        "recipient" => [
            "id" => $recipient_id
        ],
...
function setGetStartedButton() {
    global $pageAccessToken;
    $url = "https://graph.facebook.com/v21.0/me/messenger_profile?access_token=" . $pageAccessToken;
...
function getUserProfile($recipient_id) {
    global $pageAccessToken;
    $url = "https://graph.facebook.com/v21.0/" . $recipient_id . "?fields=first_name,last_name,profile_pic&access_token=" . $pageAccessToken;
...
function getLastInteractionTime($recipient_id) {
    global $pageAccessToken;
    // Define the URL for the Graph API call to fetch conversations
    $url = "https://graph.facebook.com/v21.0/me/conversations?fields=messages.limit(1){created_time,from}&access_token=$pageAccessToken";
    ...
    if ($http_code === 200) {
        ...
        // Iterate through conversations to find messages from the specified user
        foreach ($data['data'] as $conversation) {
            if (isset($conversation['messages']['data'])) {
                foreach ($conversation['messages']['data'] as $message) {
                    if ($message['from']['id'] === $recipient_id) {
                        // Return the last interaction time in UNIX timestamp
                        return strtotime($message['created_time']);
                    }
                }
            }
        }
    ...
    // Return false if no recent interaction is found
    return false;
}

function getUserOrLeaderAssignment($messageType = 'get_msg') {
    Global $db;
    
    $orderBy = ($messageType === 'get_msg') ? 'ASC' : 'DESC';

    // Get active users (team members)
    $get_users = $db->orderBy('position', $orderBy)
                    ->where('active', '1')
                    ->where('banned', '0')
                    ->where('leader_id', '0', '>') // Ignore leaders in this query
    ...
    // Calculate the number of messages for each user and accumulate per leader
    foreach ($get_users as $value) {
        $user_id = $value->user_id;
        $leader_id = $value->leader_id;

        // Get the message count for the current user within the year
        $msg_count = $db->where('user_id', $user_id)
                        ->where('msg_type', $messageType)
                        ->where('time', $year_start, '>=')
                        ->where('time', $year_end, '<=')
                        ->getValue(T_MESSAGES_ASSIGN, 'COUNT(*)');

        if ($messageType === 'lead') {
            if ($leader_id > 0) {
    	        
                if (!isset($team_members[$leader_id])) {
                    $team_members[$leader_id] = 0;
                }
                
                $team_members[$leader_id] += 1; // Increase team size by 1
                $report_leader[$leader_id] = [
                    'user_id' => $leader_id,
                    'user_count' => $team_members[$leader_id]
                ];
            }
        } else {
            if ($leader_id > 0) {
    ...
    // Calculate the percentage for each leader (messages per user)
    foreach ($report_leader as $leader_id => $value) {
        $leader_leads = $db->where('assigned', $leader_id)->where('created', $year_start, '>=')->where('created', $year_end, '<=')->getValue(T_LEADS, 'COUNT(*)');
        $leader_leads = $leader_leads ?: 0;
        $report_leader[$leader_id]['lead_count'] = $leader_leads;
        $report_leader[$leader_id]['parcent'] = $leader_leads / $value['user_count'];
    }

    // Sort the $report array by 'msg_count' in descending order
    usort($report, function ($a, $b) {
        return $a['msg_count'] <=> $b['msg_count']; // Ascending order
    });

    // Sort the leaders by their 'parcent' value (ascending order)
    usort($report_leader, function ($a, $b) {
        return $a['parcent'] <=> $b['parcent'];
    });

    if ($messageType === 'lead') {
        return [
            'report_leader' => $report_leader,
            'user_id' => $report_leader[0]['user_id'] ?? null
        ];
    } else {
        return [
            'report' => $report,
            'user_id' => $report[0]['user_id'] ?? null
        ];
    }
}

function insertMessageAssignment($threadId, $pageId, $message_count, $unread_count, $updated_time, $is_phone = false, $messageType = 'lead') {
    Global $db;
    // Use default ID for specific page IDs
    $defaultPageIds = ['9888360291179818', '8989406277822217'];
    if (in_array($pageId, $defaultPageIds)) {
        $userId = '999';
    }
    try {
        // Get user assignment based on message type
        if ($messageType == 'lead') {
            $userId = '999';
        } else {
            $get_user_data = getUserOrLeaderAssignment($messageType);
            $userId = $get_user_data['user_id'];
        }

        // Check if a valid user was assigned
        if (empty($userId)) {
            throw new Exception("No valid user ID found for message assignment.");
        }

        // Insert into messages assignment
        $result1 = $db->insert(T_MESSAGES_ASSIGN, [
            'thread_id' => $threadId,
            'user_id' => $userId,
        ...
        if (!$result1) {
            throw new Exception("Failed to insert into T_MESSAGES_ASSIGN.");
        }

        // Insert into user chats
        $result2 = $db->insert(T_U_CHATS, [
            'thread_id' => $threadId,
            'page_id' => $pageId,
            'assigned' => $userId,
            'message_count' => $message_count,
        ...
        if (!$result2) {
            throw new Exception("Failed to insert into T_U_CHATS.");
        }
        ob_clean();
        return $userId === 999 ? 1 : $userId;
    } catch (Exception $e) {
...
function updateMessageAssignment($threadId, $data_array = []) {
    // global $db;
    // $db->where('thread_id', $threadId)->update(T_MESSAGES_ASSIGN, $data_array);
}

function MessageAssignmentuser($threadId) {
    global $db;
    
    // Retrieve the user_id for the given thread_id
    $user = $db->where('thread_id', $threadId)->getOne(T_MESSAGES_ASSIGN, ['user_id']);
    
    // Ensure the query result is valid and handle the logic
    if ($user && isset($user->user_id)) {
        return $user->user_id === 999 ? 1 : $user->user_id;
    }

    // Return a default value (optional) if no user_id is found
    return 1;
}


function create_or_get_nominees($customer_id, $nominees) {
    Global $db;

    $ids = [];
    if (empty($nominees) || !is_array($nominees)) return $ids;

    foreach ($nominees as $n) {
        $n_name = isset($n['name']) ? trim($n['name']) : '';
        if ($n_name === '') continue;

        $n_relation    = isset($n['relation']) ? trim($n['relation']) : null;
        $n_address     = isset($n['address']) ? trim($n['address']) : null;
        $n_phone       = isset($n['phone']) ? trim($n['phone']) : null;
        $n_share       = isset($n['share_parcent']) ? (string)trim($n['share_parcent']) : null;
        $n_birthday_raw= isset($n['birthday']) ? $n['birthday'] : null;

        // Normalize birthday -> unix timestamp (int) or null
        $n_birthday = null;
        if (!empty($n_birthday_raw)) {
            // if it's already numeric, use it; otherwise try strtotime
            if (is_numeric($n_birthday_raw)) {
                $n_birthday = (int)$n_birthday_raw;
            } else {
                $ts = strtotime($n_birthday_raw);
                if ($ts !== false) $n_birthday = (int)$ts;
            }
        }

        // try to find existing nominee for this customer with same name+relation
        $db->where('customer_id', (int)$customer_id);
        $db->where('name', $n_name);
        if (!empty($n_relation)) $db->where('relation', $n_relation);
        $exists = $db->getOne('crm_nominees', 'id, address, phone, birthday, share_parcent');

        if ($exists && isset($exists['id'])) {
            $nominee_id = (int)$exists['id'];

            // Optionally update fields if changed (address/phone/birthday/share)
            $to_update = [];
            if ($n_address !== null && $n_address !== $exists['address']) $to_update['address'] = $n_address;
            if ($n_phone !== null && $n_phone !== $exists['phone']) $to_update['phone'] = $n_phone;
            if ($n_birthday !== null && (string)$n_birthday !== (string)$exists['birthday']) $to_update['birthday'] = $n_birthday;
            if ($n_share !== null && $n_share !== $exists['share_parcent']) $to_update['share_parcent'] = $n_share;

            if (!empty($to_update)) {
                $db->where('id', $nominee_id);
                $db->update('crm_nominees', $to_update);
            }

            $ids[] = $nominee_id;
        } else {
            // Insert new nominee — note we omit 'time' so DB default current_timestamp() applies
            $ins = [
                'customer_id'  => (int)$customer_id,
                'name'         => $n_name,
                'address'      => $n_address,
                'birthday'     => $n_birthday,         // int(11)
                'share_parcent'=> $n_share,            // varchar(32)
                'relation'     => $n_relation,
                'phone'        => $n_phone
            ];

            $newId = $db->insert('crm_nominees', $ins);
            if ($newId) $ids[] = (int)$newId;
        }
    }
...
/**
 * Create booking helper entry that references nominees
 * $booking_data keys:
 *   booking_id (required), client_id (required),
 *   status (string), per_katha, installment (string or array),
 *   nominees (array of nominee arrays) OR nominee_ids (array of ints)
 */
function create_booking_with_nominees($booking_data) {
    Global $db;

    // minimal validation
    if (empty($booking_data['booking_id']) || empty($booking_data['client_id'])) {
        return false;
    }
    ...
    // If nominees are provided as detailed arrays, create/reuse them
    if (!empty($booking_data['nominees']) && is_array($booking_data['nominees'])) {
        $nominee_ids = create_or_get_nominees($client_id, $booking_data['nominees']);
    } elseif (!empty($booking_data['nominee_ids']) && is_array($booking_data['nominee_ids'])) {
        // if the caller passes existing IDs, use them (sanitize to ints)
        $nominee_ids = array_map('intval', $booking_data['nominee_ids']);
    }

    // ensure we store a JSON string or null
    $nominee_ids_json = !empty($nominee_ids) ? json_encode(array_values($nominee_ids), JSON_UNESCAPED_UNICODE) : null;

    // Insert booking helper - adapt field names/types to your schema
    $insert = [
        'booking_id'  => $booking_data['booking_id'],
        'client_id'   => $client_id,
        'status'      => isset($booking_data['status']) ? $booking_data['status'] : '0',
        'time'        => isset($booking_data['time']) ? $booking_data['time'] : date('Y-m-d H:i:s'),
        'nominee_ids' => $nominee_ids_json,
        'per_katha'   => isset($booking_data['per_katha']) ? $booking_data['per_katha'] : null,
        'installment' => isset($booking_data['installment']) ? $booking_data['installment'] : null
    ];

    $ins = $db->insert('wo_booking_helper', $insert);
    if ($ins) return $ins;
    return false;
}
