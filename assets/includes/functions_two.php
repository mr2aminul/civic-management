...
function Wo_PageData($page_id = 0) {
    ...
    $fetched_data["page_id"]           = $fetched_data["page_id"];
    $fetched_data["type"]              = "page";
...
function Wo_CountAllData($type) {
    ...
    $type_id   = Wo_Secure($type_id);
	if ($type == "user") {
		$query_one = mysqli_query($sqlConnect, "SELECT COUNT($type_id) as count FROM {$type_table} WHERE `admin` <> '1'");
	} else {
...
function Wo_CountUser($type) {
    global $db;
	$count = 0;
	if ($type == 'male') {
		$count = $db->where('gender', 'male')->getValue(T_USERS, 'COUNT(*)');
	} else if ($type == 'female') {
		$count = $db->where('gender', 'female')->getValue(T_USERS, 'COUNT(*)');
	} else if ($type == 'not_active') {
		$count = $db->where('active', '0')->getValue(T_USERS, 'COUNT(*)');
	} else if ($type == 'active') {
		$count = $db->where('active', '1')->getValue(T_USERS, 'COUNT(*)');
	}
	return $count;
}
...
function Wo_SendMessage($data = array()) {
    ...
    if (isset($data["insert_database"])) {
        if ($data["insert_database"] == 1) {
            $user_id   = Wo_Secure($wo["user"]["user_id"]);
            $query_one = mysqli_query($sqlConnect, "INSERT INTO " . T_EMAILS . " (`email_to`, `user_id`, `subject`, `message`, `project`, `name`, `katha`, `phone`, `email`, `address`, `profession`) VALUES ('{$to_email}', '{$user_id}', '{$subject}', '{$message_body}', '{$project}', '{$name}', '{$katha}', '{$phone}', '{$email}', '{$address}', '{$profession}')");
            if ($query_one) {
                return true;
            }
        }
...
function Wo_RegisterCareer($data = array()) {
    global $wo, $sqlConnect;
	...
	$query_one = mysqli_query($sqlConnect, "INSERT INTO " . T_CAREER . " (`name`, `phone`, `position`, `sub_position`, `email`, `message`, `postPhotos`) VALUES ('{$name}', '{$phone}', '{$position}', '{$sub_position}', '{$email}', '{$message}', '{$postPhotos}')");
	if ($query_one) {
		return true;
	}
}
function Wo_DeleteCareer($career_id = "") {
    global $wo, $sqlConnect;
    if ($wo["loggedin"] == false) {
        return false;
    }
    $career_id = Wo_Secure($career_id);
    $query     = mysqli_query($sqlConnect, "DELETE FROM " . T_CAREER . " WHERE `id` = {$career_id}");
    if ($query) {
        return true;
    }
}
...
function update_lockout_time() {
	...
	return $_SESSION['lockout_timestamp'];
}
function reset_working_status() {
    global $db; // Assuming $db is your database connection object
    $last_activity_threshold = time() - 180; // Threshold: 180 seconds (3 minutes) ago

    // Fetch leads that have status = 3 and last_activity <= $last_activity_threshold
    $leads = $db->where('status', 3)
                ->where('last_activity', $last_activity_threshold, '<=')
                ->get(T_LEADS, 50); // Fetching up to 50 leads
	if ($leads) {
		foreach ($leads as $lead) {
			$status = '';

			// Access object properties using -> notation
			...
			if ($assigned == 0) {
				$status = 0;
			} else if ($assigned > 0 && !empty($remarks)) {
				$status = 1;
			} else if ($assigned > 0 && empty($remarks)) {
				$status = 0;
			}

			if (!empty($remarks) && !in_array($remarks, ['ready plot', 'not interest', 'sale complete'])) {
				$status = 4;
			}

			// Update the lead status
			$update = $db->where('lead_id', $lead_id)->update(T_LEADS, array(
				'status' => $status
			));
		}
}
...
function update_lockout($input = '') {
	global $wo;
    if (isset($input) && !empty($input)) {
		
        if ($wo['user']['manage_pass'] == $input) {
            $_SESSION['lockout'] = 'no';
            update_lockout_time();
            return true;
        } else {
			$_SESSION['lockout'] = null;
			$_SESSION['lockout_timestamp'] = null;
            return false;
        }
    } else {
        return false;
...
function is_lockout() {
	...
	if (isset($_SESSION['lockout']) && isset($_SESSION['lockout_timestamp']) && time() - $_SESSION['lockout_timestamp'] <= $lockout_duration && $_SESSION['lockout'] == 'no') {
		return false;
	} else {
		$_SESSION['lockout'] = null;
		$_SESSION['lockout_timestamp'] = null;
...
function Wo_MessageType($content = '', $type = '') {
    // Define file types and their corresponding messages
    $media_types = [
        'image' => ['jpg', 'jpeg', 'png', 'gif'],
        'video' => ['mp4', 'mov', 'avi', 'mkv'],
        'document' => ['pdf', 'doc', 'docx', 'xlsx', 'ppt', 'txt'],
        'audio' => ['mp3', 'wav', 'ogg', 'aac'],
    ];
	
    // Check for specific types
    if ($type === 'media' || $type === 'stickers') {
        $file_extension = strtolower(pathinfo($content, PATHINFO_EXTENSION)); // Get file extension

        foreach ($media_types as $category => $extensions) {
            if (in_array($file_extension, $extensions)) {
                if ($category === 'image' && $type === 'stickers') {
                    return 'Send a sticker'; // Special case for stickers
                }
				if ($type === 'stickers') {
                    return 'Send a sticker'; // Special case for stickers
                }
                return 'Send a ' . $category;
            }
        }
        ...
        return 'Send a file'; // Default fallback if no match
    }

    // Handle non-media or stickers content
    $arr_to_replace = [
        'Manage calls(' => '<span class="text-red"><svg fill="red" width="18px" height="180px" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg" data-name="Layer 1" style="&#10;width: 18px;&#10;height: 18px;&#10;"><path d="M6,7.49a1,1,0,0,0,1-1V5.9L9.88,8.78a3,3,0,0,0,4.24,0l4.59-4.59a1,1,0,0,0,0-1.41,1,1,0,0,0-1.42,0L12.71,7.36a1,1,0,0,1-1.42,0L8.41,4.49H9a1,1,0,0,0,0-2H6a1,1,0,0,0-.92.61A1.09,1.09,0,0,0,5,3.49v3A1,1,0,0,0,6,7.49Zm15.94,7.36a16.27,16.27,0,0,0-19.88,0,2.69,2.69,0,0,0-1,2,2.66,2.66,0,0,0,.78,2.07L3.6,20.72A2.68,2.68,0,0,0,7.06,21l.47-.32a8.13,8.13,0,0,1,1-.55,1.85,1.85,0,0,0,1-2.3l-.09-.24a10.49,10.49,0,0,1,5.22,0l-.09.24a1.85,1.85,0,0,0,1,2.3,8.13,8.13,0,0,1,1,.55l.47.32a2.58,2.58,0,0,0,1.54.5,2.72,2.72,0,0,0,1.92-.79l1.81-1.82A2.66,2.66,0,0,0,23,16.83,2.69,2.69,0,0,0,21.94,14.85ZM20.8,17.49,19,19.3a.68.68,0,0,1-.86.1c-.19-.14-.38-.27-.59-.4a11.65,11.65,0,0,0-1.09-.61l.4-1.09a1,1,0,0,0-.6-1.28,12.42,12.42,0,0,0-8.5,0,1,1,0,0,0-.6,1.28l.4,1.1a9.8,9.8,0,0,0-1.1.6l-.58.4A.66.66,0,0,1,5,19.3L3.2,17.49A.67.67,0,0,1,3,17a.76.76,0,0,1,.28-.53,14.29,14.29,0,0,1,17.44,0A.76.76,0,0,1,21,17,.67.67,0,0,1,20.8,17.49Z"/></svg> Missed audio call</span>',
        'You missed a call from' => '<span class="text-red"><svg fill="red" width="18px" height="180px" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg" data-name="Layer 1" style="&#10;width: 18px;&#10;height: 18px;&#10;"><path d="M6,7.49a1,1,0,0,0,1-1V5.9L9.88,8.78a3,3,0,0,0,4.24,0l4.59-4.59a1,1,0,0,0,0-1.41,1,1,0,0,0-1.42,0L12.71,7.36a1,1,0,0,1-1.42,0L8.41,4.49H9a1,1,0,0,0,0-2H6a1,1,0,0,0-.92.61A1.09,1.09,0,0,0,5,3.49v3A1,1,0,0,0,6,7.49Zm15.94,7.36a16.27,16.27,0,0,0-19.88,0,2.69,2.69,0,0,0-1,2,2.66,2.66,0,0,0,.78,2.07L3.6,20.72A2.68,2.68,0,0,0,7.06,21l.47-.32a8.13,8.13,0,0,1,1-.55,1.85,1.85,0,0,0,1-2.3l-.09-.24a10.49,10.49,0,0,1,5.22,0l-.09.24a1.85,1.85,0,0,0,1,2.3,8.13,8.13,0,0,1,1,.55l.47.32a2.58,2.58,0,0,0,1.54.5,2.72,2.72,0,0,0,1.92-.79l1.81-1.82A2.66,2.66,0,0,0,23,16.83,2.69,2.69,0,0,0,21.94,14.85ZM20.8,17.49,19,19.3a.68.68,0,0,1-.86.1c-.19-.14-.38-.27-.59-.4a11.65,11.65,0,0,0-1.09-.61l.4-1.09a1,1,0,0,0-.6-1.28,12.42,12.42,0,0,0-8.5,0,1,1,0,0,0-.6,1.28l.4,1.1a9.8,9.8,0,0,0-1.1.6l-.58.4A.66.66,0,0,1,5,19.3L3.2,17.49A.67.67,0,0,1,3,17a.76.76,0,0,1,.28-.53,14.29,14.29,0,0,1,17.44,0A.76.76,0,0,1,21,17,.67.67,0,0,1,20.8,17.49Z"/></svg> Missed audio call</span>',
        'https://www.google.com/maps/' => '<span class="text-red"><svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" viewBox="0 0 5644.173 5644.173" style="&#10;width: 18px;&#10;height: 18px;&#10;"> <defs> <path id="a" d="M4300.322 0c-439.439 0-829.828 210.985-1075.081 537.54H403.155C181.42 537.54 0 718.96 0 940.695v4300.322c0 221.736 181.42 403.155 403.155 403.155h4300.322c221.735 0 403.155-181.419 403.155-403.155V2609.086c282.208-412.562 537.54-784.137 537.54-1265.235C5644.173 601.373 5042.8 0 4300.322 0z"/> <path id="m" fill="#DB4437" d="M4300.322 0c-742.478 0-1343.851 601.373-1343.851 1343.851 0 1012.591 1130.85 1540.053 1264.563 2985.364 4.031 40.315 38.3 71.896 79.287 71.896s75.927-31.581 79.287-71.896c133.713-1445.312 1264.563-1972.773 1264.563-2985.364C5644.173 601.373 5042.8 0 4300.322 0z"/> <clipPath id="c"> <use overflow="visible" style="transform:scaleX(1.6) translateX(-31.5%)" xlink:href="#a"/> </clipPath> <filter id="b" width="280%" height="280%" x="-70%" y="-70%"> <feGaussianBlur in="SourceAlpha" stdDeviation="600"/> <feComponentTransfer> <feFuncA exponent=".7" type="gamma"/> </feComponentTransfer> <feOffset dx="300" dy="100"/> </filter> </defs> <path fill="#0F9D58" d="M2553.316 3090.856l2268.569-2535.46c-37.479-11.6-77.248-17.856-118.408-17.856H403.155C181.42 537.54 0 718.96 0 940.695v4300.322c0 41.16 6.257 80.929 17.856 118.408l2535.46-2268.569z"/> <path fill="#4285F4" d="M2553.316 3090.856L284.747 5626.316c37.479 11.6 77.248 17.856 118.408 17.856h4300.322c41.16 0 80.929-6.256 118.408-17.856l-2268.569-2535.46z"/> <path fill="#D2D2D2" d="M2553.316 3090.856l2535.46 2268.569c11.6-37.479 17.856-77.248 17.856-118.408V940.695c0-41.16-6.257-80.929-17.856-118.408l-2535.46 2268.569z"/> <path fill="#F1F1F1" d="M5106.633 5241.018L2687.701 2822.086l-470.348 403.155 2418.931 2418.931h67.192c221.737.001 403.157-181.419 403.157-403.154z"/> <path fill="#FFDE48" d="M4703.478 537.54L0 5241.018c0 221.736 181.42 403.155 403.155 403.155h67.193l4636.285-4636.285v-67.193c0-221.735-181.42-403.155-403.155-403.155z"/> <path fill="#FFF" fill-opacity=".2" d="M4703.478 537.54H403.155C181.42 537.54 0 718.96 0 940.695v33.596c0-221.735 181.42-403.155 403.155-403.155h4300.322c221.735 0 403.155 181.42 403.155 403.155v-33.596c.001-221.735-181.419-403.155-403.154-403.155z"/> <path fill="#263238" fill-opacity=".1" d="M4703.478 5610.577H403.155C181.42 5610.577 0 5429.157 0 5207.421v33.596c0 221.736 181.42 403.155 403.155 403.155h4300.322c221.735 0 403.155-181.419 403.155-403.155v-33.596c.001 221.736-181.419 403.156-403.154 403.156z"/> <path fill="#EEE" d="M1142.273 1545.428v286.24h397.78c-31.581 169.997-180.748 293.631-397.78 293.631-241.221 0-437.423-204.265-437.423-444.814s196.202-444.814 437.423-444.814c108.852 0 205.609 37.628 282.881 110.196l211.657-211.656c-128.338-120.275-294.975-193.515-494.537-193.515-408.531 0-739.118 330.587-739.118 739.118s330.587 739.118 739.118 739.118c426.672 0 709.553-300.351 709.553-722.32 0-52.41-4.703-102.805-13.438-151.183h-696.116z"/> <use clip-path="url(#c)" filter="url(#b)" opacity=".25" style="transform:scaleX(.625) translateX(50.4%)" xlink:href="#m"/> <use xlink:href="#m"/> <circle cx="4300.322" cy="1343.851" r="470.348" fill="#7B231E"/> <path fill="#FFF" fill-opacity=".2" d="M4300.322 33.596c735.758 0 1333.1 591.294 1343.179 1324.365 0-4.703.672-9.407.672-14.11C5644.173 601.373 5042.8 0 4300.322 0s-1343.85 601.373-1343.85 1343.851c0 4.703.672 9.407.672 14.11 10.078-733.07 607.42-1324.365 1343.178-1324.365z"/> <path fill="#3E2723" fill-opacity=".2" d="M4379.609 4295.619c-3.36 40.315-38.3 71.896-79.287 71.896s-75.927-31.581-79.287-71.896C4088.666 2857.027 2967.894 2327.55 2957.144 1324.365c0 6.719-.672 12.767-.672 19.486 0 1012.591 1130.85 1540.053 1264.564 2985.364 4.031 40.315 38.3 71.896 79.287 71.896s75.927-31.581 79.287-71.896c133.712-1445.312 1264.563-1972.773 1264.563-2985.364 0-6.719-.672-12.767-.672-19.486-10.751 1003.184-1130.851 1533.333-1263.892 2971.254z"/> <radialGradient id="r" cx="140.0966" cy="5533.9414" r="6883.6064" gradientTransform="matrix(1 0 0 -1 0 6182.7524)" gradientUnits="userSpaceOnUse"> <stop offset="0" stop-color="#fff" stop-opacity=".1"/> <stop offset="1" stop-color="#fff" stop-opacity="0"/> </radialGradient> <path fill="url(#r)" d="M4300.322 0c-439.439 0-829.828 210.985-1075.081 537.54H403.155C181.42 537.54 0 718.96 0 940.695v4300.322c0 221.736 181.42 403.155 403.155 403.155h4300.322c221.735 0 403.155-181.419 403.155-403.155V2609.086c282.208-412.562 537.54-784.137 537.54-1265.235C5644.173 601.373 5042.8 0 4300.322 0z"/> </svg> Send google map link</span>',
    ];

    foreach ($arr_to_replace as $key => $replacement) {
        if (strpos($content, $key) !== false) {
            return $replacement;
        }
    }

    $content = html_entity_decode(Wo_Emo($content));
