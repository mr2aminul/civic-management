...
$allowedDomains = [
    ...
    ['192.168.0.2', 'vrbel', 'civicbd_menu.php']
];

function getDomainInfo(array $allowedDomains) {
    ...
    // Determine the schema (http or https)
    ...
    foreach ($allowedDomains as $domainConfig) {
        if ($currentHost === $domainConfig[0]) {
            return [
                ...
                'theme' => $domainConfig[1],
                'menu_file' => $domainConfig[2],
            ];
        }
    }

    // If the host is not in the allowed domains, return false
    return false;
}
...
$domain_details = $wo['domain_details'] = getDomainInfo($allowedDomains);
...
$wo['get_apis'] = $wo['config']['get_apis_key'];
...
function calculateTotalLeaves($user_id = null) {
    ...
    // Initialize total leaves
    $total_leaves = 0;

    // Ensure joining date is valid
    if (isset($user["joining_date"]) && $user["joining_date"] !== '0000-00-00' && !empty($user["joining_date"])) {
        try {
            ...
            $interval = $currentDate->diff($joiningDate);
...
include_once("assets/libraries/facebook-graph/vendor/autoload.php");
...
$userAccessToken = 'EAAHq1kyxkCYBO2bZA4UGI3G1zvOZAgD6ZAE8hegsEii7vKe3jZCdzBlZCwRhCpRnvxMkMsWCNJaFBcp7CLIHK33gApvWOpLLNXFkJUgWUGyfG4PSsaTtGPAi4iBGWZCDMnFqdi7dIGco1vloynB8hZCKsom5qXGkhdZB0TZCM6KEGZBEadwjVN71XPvqyN';
$pageAccessToken = 'EAAHq1kyxkCYBO9fvgeEEvz392TgCOzrTRUvQzhumF9Ry0JSQ1cA6FviyyTnG1OnrwU8epj6uyg0asBpMlBgYZBZCGzOcZAsSI9ZCgbGYI0lL5LK2yqfG5K1cPmDFZCo304Sa2o4bRaz1kHZAgzqztNCkOvlwL5SMYst6YcqxyF5WirVdBWRt9nZBLdZAKKzxPp8B';


$wo['total_leaves'] = 12;

require_once "assets/libraries/web-push/vendor/autoload.php";
